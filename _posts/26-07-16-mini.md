...
2. `기울기(gradient)` 산출<br>
  미니배치의 손실함수 값을 줄이기 위해 손실함수의 기울기를 구한다.<br>
3. `매개변수` 갱신<br>
  매개변수를 기울기 반대 방향으로 학습률만큼 갱신시킨다.
4. 반복<br>
  위 1~3단계를 반복한다.
...
for i in range(iters_num):
  ...
  start_idx = (i % iter_per_epoch) * batch_size
  batch_mask = indices[start_idx : start_idx + batch_size]
...
![Desktop View](/assets/img/26-07-16/accgraph.png){: height="589"}

이 그림은 신경망의 정확도를 나타낸다.<br>
`train acc`는 훈련의 정확도를, `test acc`는 시험의 정확도를 나타낸다.

### '에폭(Epoch)'
에폭은 학습에서 훈련 데이터를 모두 소진했을 때의 횟수를 의미하는 단위이다.
예를 들어, 훈련 데이터가 1000개, 미니배치 크기가 10개라고 설정하면 이 데이터들을 학습하기 위해서는 100번의 반복 이 이루어져야 한다. 100번의 반복이 다 이루어졌을 경우 1`에폭(Epoch)`이 완료된 것이다.

에폭 단위를 사용하여 학습 정도를 확인할 수 있다. 신경망은 앞서 말했듯이 훈련을 과도하게 하면 `오버피팅`이 일어나 학습한 데이터와 다른 데이터 즉,실전에서는 성능이 떨어질 수 있다.

위 그림과 같이 `test acc`를 통해 학습되지 않은 데이터에서의 성능을 알 수 있었다.
